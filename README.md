...
**Press + Turn Fires Once Per Press**: an option for the Press + Turn gesture (Keypress mode, or any mode once a hold key is set) that turns it into a flick — hold the button, one nudge left or right, release, one keypress — instead of repeating the key once per detent. Useful for toggling something such as a quick-note window, which repeating would undo. The menu item sets the default; each app in **Configure Applications...** has its own Press + Turn Fires Once Per Press checkbox, shown while that app is in Keypress mode or has a hold key (a newly added app starts with it off).
