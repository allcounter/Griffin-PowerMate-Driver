...
- **Hold key** → holds a configured key down for exactly as long as the PowerMate button is held (including a bare modifier such as **Fn**, or a hardware media/function key such as **Volume**, **Brightness**, **Mute**, or **Play/Pause**), for push-to-talk dictation and anything else that reacts to a key being held. A short tap still resolves as Click (or Double-click, if configured). The key engages after a roughly 0.2 s arming delay, or immediately when Click and Double-click are both None. Long press is unreachable while a hold key is set. The hold key is a global setting: it applies in every app, including ones that have their own **Configure Applications** override. **Press + Turn Fires Once Per Press** sends the Press + Turn key once for the whole press instead of once per detent — a flick (hold, one nudge, release) useful for toggling something such as a quick-note window, which repeating would undo.
...
**Hold key**: a new push-to-talk action. Configure a key (including a bare modifier such as Fn, or a hardware media/function key such as Volume, Brightness, Mute, or Play/Pause) via **Hold key** → **Hold Key While Pressed...**, and it's held down for exactly as long as the PowerMate button is held. A short tap still resolves as Click (or Double-click, if configured); the key engages after a roughly 0.2 s arming delay, or immediately when Click and Double-click are both None. Long press is unreachable while a hold key is set, and Press + Turn sends the Keypress-mode "Press + Turn" key in every mode instead of skipping tracks, cancelling the hold so you can step through just-dictated text. The hold key is global rather than per-app: apps with their own **Configure Applications** override use it too, and lose their long press for as long as one is set.
