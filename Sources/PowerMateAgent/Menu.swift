...
final class MenuHandler: NSObject, NSMenuDelegate {
    ...
    @objc func showHelp() {
        let lines: [(String, String)] = [
            ...
            ("Hold key", " – Holds a single key down for exactly as long as the PowerMate button is held, for push-to-talk dictation and anything else that reacts to a key being held. A bare modifier such as Fn can be recorded, and so can a hardware media/function key such as Volume, Brightness, Mute, or Play/Pause. A short tap still performs the Click action (and a double-click its action, if one is set); the key engages once the button has been held for 0.2 s. With Click and Double-click both None it engages immediately. Long press does nothing while a hold key is set. Press + Turn sends the Keypress-mode \"Press + Turn\" key in every mode (instead of skipping tracks) and cancels the hold, so you can step through just-dictated text.\n"),
            ("Modifiers:", ""),
